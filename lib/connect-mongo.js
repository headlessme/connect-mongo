...
module.exports = function(connect) {
  ...
  MongoStore.prototype.get = function(sid, callback) {
    ...
    function doUnserialize(session) {
      if (session) {
        if (!session.expires || new Date < session.expires) {
          callback(null, self._unserialize_session(session.session ? session.session : session.data));
        } else {
  ...
  MongoStore.prototype.set = function(sid, session, callback) {
    try {
      var s = {_id: sid, sid:sid+'-deprecated', session: this._serialize_session(session)};
