...
module.exports = function(connect) {
          ...
          if (err) {
            ...
            if (session) {
              if (!session.expires || new Date < session.expires) {
                callback(null, self._unserialize_session(session.data));
              } else {
  ...
  MongoStore.prototype.set = function(sid, session, callback) {
    try {
      // added for temporary transition to module! DO NOT PUSH THIS UPSTREAM
      var ses = this._serialize_session(session);
      var s = {_id: sid, data: ses, session:ses };
